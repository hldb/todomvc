...
const token = import.meta.env.VITE_W3_TOKEN;
let started = false;
...
let helia: Helia<any>, libp2p: Libp2p<any>, welo: Welo, manifest: Manifest, db: Database, model: TodoModel;
...
export async function start(): Promise<void> {
	...
	const initialDownload = async (num: number) => {
		if (num === 3) {
			...
			libp2p.addEventListener('self:peer:update', (event) => initialDownload(event.detail.peer.addresses.length), {
				once: true,
			});
		}
	};
...
const throttledUpload = async (db: Database): Promise<void> => {
	const _uploadChanges = () =>
		uploadChanges(db).then(() => {
			uploading = null;
			if (nextUpload != null) {
