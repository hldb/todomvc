...
/* global cy, Cypress */
const framework = 'typescript-react';
if (!framework) {
	throw new Error(
		`
	...
	describe(title, function () {
		...
		beforeEach(function () {
			...
			cy.visit('/', {
				onBeforeLoad: (win) => {
					...
					win.EventTarget.prototype.addEventListener = function (name) {
						if (['change', 'keydown', 'keypress', 'keyup', 'input'].includes(name)) {
			...
			cy.document().then((doc) => {
				if (framework in usesIDSelectors) {
					...
					throw new Error('Cannot determine what kind of selectors this app uses. Add it to usesIDSelectors.');
					...
					throw new Error('Cannot determine what kind of selectors this app uses.');
				}
			});
		});
		...
		context('Editing', function () {
			...
			it('should remove the item if an empty text string was entered', function () {
				...
				cy.get(selectors.todoItems).eq(1).find('.edit').clear().type('{enter}').then(safeBlur);
